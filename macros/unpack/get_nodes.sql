...
{%- macro default__get_nodes() -%}
    ...
    {%- if execute -%}
    {%- set node_path_filter = env_var('DPE_NODE_PATH_FILTER', None) -%}
    {%- set nodes_list = graph.nodes.values() -%}
    {%- set values = [] -%}

    {%- for node in nodes_list -%}
        {%- if node_path_filter is None or node_path_filter in node.path -%}
            {{ log('Evaluating node: ' ~ node.name, info=True) }}
            {%- set values_line  =
                [
